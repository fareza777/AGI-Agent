...
_GIT_READ_SUBCMDS = {"status", "log", "diff", "branch", "show", "remote",
                     "ls-files", "shortlog"}
_GIT_WRITE_SUBCMDS = {"add", "commit", "checkout", "restore", "reset",
                      "stash", "tag", "fetch", "pull", "push", "merge",
                      "rebase", "cherry-pick", "rm"}
...
def _git_flag_blocked(parts: list) -> str:
    ...
    if lowered and lowered[0] == "clean" and any(
        t in lowered for t in ("-fd", "-fdx", "-df", "-f")
    ):
...
def git(repo_path: str, command: str) -> str:
    ...
    if sub in _GIT_READ_SUBCMDS:
        ...
        if config.GIT_READ_ONLY:
            return (f"git '{sub}' is a write command but git tool is in "
                    f"read-only mode (ENGRAM_GIT_READ_ONLY=1). Flip the env "
                    f"var to 0 in .env and restart to allow writes.")
        mode = "write"
    else:
        return (f"git subcommand '{sub}' is not in the allowed list. "
